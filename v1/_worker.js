...
async function fetchViaSocket(targetUrl, { ua, referer, range } = {}) {
  ...
  while (headerEnd === -1) {
    ...
    if (headerEnd === -1 && buf.length > 131072) throw new Error('响应头过大或格式异常');
  }
  if (headerEnd === -1) throw new Error('未收到有效的 HTTP 响应(连接可能被拒绝或超时)');

  const headerText = new TextDecoder('utf-8').decode(buf.slice(0, headerEnd - 4));
  let bodyStart = buf.slice(headerEnd);
